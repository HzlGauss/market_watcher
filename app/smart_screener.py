...
策略：稳健 / 低位埋伏。核心逻辑是「先锁定今日资金与题材共同聚焦的热点板块，
再在这些板块内找低位滞涨 + 主力持续低吸 + 尚未启动的个股」——板块有资金关注
（题材催化），个股却还在低位横盘、主力悄悄吸筹，等板块启动时补涨。

五阶段管线：
  Stage 0  市场背景采集（妙想新闻题材 + 龙虎榜板块流）
  Stage 1  LLM 综合判断 → 热点板块 + 妙想选股条件（function calling，强制结构化）
  Stage 2  妙想 stock_screen 执行 → 候选池（跨条件去重 + 共振加分）
  Stage 3  主力资金持续低吸评分 + 多维过滤（黑名单板块/概念 + ST/退市 + 新股/次新股，核心）
  Stage 4  LLM 综合排序解读 → 强关注/关注/风险分级
  Stage 5  报告生成 + 保存 + ServerChan 推送
...
def _build_stage1_prompt(context: str, blacklist: list[str], count: int) -> str:
    lines = [
        "你是一名A股低位埋伏策略师。请根据以下市场背景，判断当前热点板块并给出妙想智能选股条件。",
        "",
...
def _score_accumulation(
    ...
    industry_map: dict[str, str] = {}
    listing_map: dict[str, str] = {}
    if blacklist_sectors or blacklist_concepts:
        try:
    ...
    for c in kept:
        ...
        try:
            klines = technical.fetch_historical_kline(c.code, c.market, days=window)
...
def _build_stage4_prompt(top: list[ScreeningCandidate], hot_sectors: list[str]) -> str:
    lines = [
        "以下是机器筛出的「低位吸筹」候选（已按吸筹分排序）。"
        "请结合所属板块持续性、技术位置、资金持续低吸强度、龙虎榜形态，做综合排序与分级。",
        "",
    ]
    if hot_sectors:
        lines.append(f"【今日热点板块】{'、'.join(hot_sectors)}")
        lines.append("")
    lines.append("【候选清单】")
    for c in top:
        acc = c.accumulation
        ...
        lines.append(
            f"- {c.name}({c.code}) 板块={sector} 吸筹分={acc.score:.1f} "
            f"净流入{acc.inflow_days}天 连续{acc.consecutive_days}天 背离度={div} "
            f"窗口涨幅={pchg} 技术={tech}"
        )
    lines.append("")
    lines.append("请输出「强关注 / 关注 / 风险」三级清单，每只附一句理由和一个可量化的下一交易日验证标准。")
    return "\n".join(lines)


def _analyze_and_rank(
    config: Config, candidates: list[ScreeningCandidate], hot_sectors: list[str]
) -> str:
    ...
    prompt = _build_stage4_prompt(top, hot_sectors)
    try:
        from app.llm_client import get_llm_client, SYSTEM_PROMPTS
        llm = get_llm_client(config)
        resp = llm.chat(
...
def _build_markdown(report: ScreeningReport) -> str:
    lines = [
        "# 🎯 智能选股（热点板块 · 低位埋伏 · 主力持续低吸）",
        "",
        f"**日期**: {report.date}",
        "**策略**: 先锁热点板块 → 板块内找「低位滞涨 + 主力持续低吸」的补涨标的",
    ]
    if report.degraded:
    ...
    if not report.candidates:
        ...
        lines.append("| 排名 | 代码 | 名称 | 板块 | 吸筹分 | 评级 | 背离度 | 净流入天数 |")
        lines.append("|------|------|------|------|--------|------|--------|-----------|")
        for c in report.candidates:
            ...
            sector = "、".join(c.hot_sectors) or (c.industry.split("-")[0] if c.industry else "—")
            lines.append(f"| {c.rank} | {c.code} | {c.name} | {sector} | {score} | {label} | {div} | {inflow} |")
        lines.append("")
...
def run_smart_screening(config: Config) -> ScreeningReport:
    ...
    """
    log.info("========== 智能选股（热点板块 · 低位埋伏 · 主力持续低吸） ==========")
    ...
    log.info(f"Stage 1 热点板块: {hot_sectors or '（未判断）'} | 条件数: {len(conditions)}")

    # Stage 2：妙想选股
    candidates = _run_mx_screening(config, conditions)
    log.info(f"Stage 2 妙想选股: {len(candidates)} 只候选")
    ...
    log.info(f"Stage 3 评分完成: {len(candidates)} 只（已过滤黑名单/ST/次新股）")

    # Stage 4：LLM 综合解读
    report.llm_analysis = _analyze_and_rank(config, candidates, hot_sectors)
